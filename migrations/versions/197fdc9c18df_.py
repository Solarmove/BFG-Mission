...
Revision ID: 197fdc9c18df
Revises: 
Create Date: 2025-08-03 22:45:14.895930
...
def upgrade() -> None:
    ...
    sa.Column('create_task_prompt', sa.TEXT(), nullable=True),
    sa.Column('manage_task_prompt', sa.TEXT(), nullable=True),
    sa.Column('work_schedule_prompt', sa.TEXT(), nullable=True),
    sa.Column('category_prompt', sa.TEXT(), nullable=True),
    ...
    sa.Column('hierarchy_level_id', sa.BIGINT(), nullable=True),
    sa.ForeignKeyConstraint(['hierarchy_level_id'], ['hierarchy_levels.id'], ),
    sa.PrimaryKeyConstraint('id'),
    ...
    sa.Column('completed_datetime', postgresql.TIMESTAMP(timezone=True), nullable=True),
    ...
    sa.Column('status', postgresql.ENUM('NEW', 'IN_PROGRESS', 'COMPLETED', 'CANCELED', 'OVERDUE', name='taskstatus'), nullable=False),
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('user_id', 'date', name='unique_user_id_date_work_schedule')
    )
    op.create_table('task_control_points',
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
    sa.Column('task_id', sa.INTEGER(), nullable=False),
    sa.Column('deadline', postgresql.TIMESTAMP(timezone=True), nullable=False),
    sa.Column('datetime_complete', postgresql.TIMESTAMP(timezone=True), nullable=True),
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.ForeignKeyConstraint(['task_control_point_id'], ['task_control_points.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['task_id'], ['tasks.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('task_report_contents',
    sa.Column('id', sa.INTEGER(), autoincrement=True, nullable=False),
